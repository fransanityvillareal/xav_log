...
Future<String> getDatabasePath() async {
  ...
  final dbPath = '${documentsDir.path}/grades.db';
...
Future<double> calculateFinalGrade(int subjectId) async {
  ...
  final rootPartitions = db.select('''
  ...
  for (final partitionRow in rootPartitions) {
    final partitionGrade = await _calculatePartitionGrade(db, partitionRow['id'] as int);
...
Future<double> _calculatePartitionGrade(Database db, int partitionId) async {
  ...
  if (subPartitions.isNotEmpty) {
    ...
    for (final subPartition in subPartitions) {
      final subGrade = await _calculatePartitionGrade(db, subPartition['id'] as int);
...
Future<void> saveComponent({required double? score, required double maxScore, required int partitionId}) async {
  final dbPath = await getDatabasePath();
  final db = sqlite3.open(dbPath);
...
Future<List<Map<String, String>>> loadPartitionsForSubject(int subjectId) async {
  final dbPath = await getDatabasePath();
  final db = sqlite3.open(dbPath);
  final result = db.select('SELECT * FROM partition WHERE subject_id = ?', [subjectId]);
  db.dispose();
  // Convert all values to String for compatibility with SubjectPartitioningScreen
  return result.map((row) => Map<String, String>.fromEntries(row.entries.map((e) => MapEntry(e.key, e.value?.toString() ?? '')))).toList();
}
